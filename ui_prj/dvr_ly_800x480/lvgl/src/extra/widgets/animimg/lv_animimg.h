...
#endif
#if LV_USE_LZ4_INTERNAL == 0
#error "lv_animimg: lz4 is required. Enable it in lv_conf.h (LV_USE_LZ4_INTERNAL 1)"
#endif
...
typedef struct {
    ...
    lv_img_dsc_t imgdsc;
    uint32_t video_max_data_decompress;
} lv_animimg_t;
 ...
 * Set the image animation images source.
 * @param img pointer to an animation image object
 * @param video_file_path the path of the video file
 */
lv_res_t lv_animimg_set_video_file_src(lv_obj_t * img, const char * video_file_path);
