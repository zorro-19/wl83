...
int mic_record_start(int sample_rate, u32 msec)
{
    int err = -1;
...
void mic_record_stop(void)
{
    ...
    /*sock_set_quit(tx_talk->sock);*/
    sock_unreg(tx_talk->sock);
    if (tx_talk) {
        free(tx_talk);
        tx_talk = NULL;
    }
...
void init_sock_receive_voice(void)
{
    ...
    if (sock_set_reuseaddr(tx_talk->sock)) {
        printf("\n [ERROR] %s - %d\n", __func__, __LINE__);
    }
    ...
    /* 建立MIC数据线程 */
    thread_fork("send_mic_data_handler", 25, 2 * 1024, 0, &tx_talk->thread_pid, send_mic_data_handler, NULL);
}
