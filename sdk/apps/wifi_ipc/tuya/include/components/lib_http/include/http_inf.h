 ...
 * @param[in/out] req
 * @param[in/out] url
 * @param[in/out] tls_config
 * @param[in/out] field_flags
 * @param[in/out] callback
 * @param[in/out] p_decode_key
 * @param[in/out] pri_data
 * @return OPERATE_RET
 */
 ...
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
#define http_inf_client_get_file(url, callback, pri_data, file_offset, file_size) \
 ...
 * @param[in] file_offset Offset of file content
 * @param[in] file_size Max size to get from file
 * @param[in] session_persistence auto handle session persistence with the same host and port
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
...
#define http_inf_client_post_field(url, callback, data, len, \
                                    add_head_cb, add_head_data, \
                                    p_decode_key, pri_data, field_flags) \
        http_inf_client_post_field_session(url, callback, data, len, \
                                    add_head_cb, add_head_data, \
                                    p_decode_key, pri_data, field_flags, FALSE)

#define http_inf_client_post_session(url, callback, data, len, \
                                    add_head_cb, add_head_data, \
                                    p_decode_key, pri_data, session_persistence) \
        http_inf_client_post_field_session(url, callback, data, len, \
                                    add_head_cb, add_head_data, \
 ...
 * @param[in] session_persistence Indicate session persistence support
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
 ...
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
#define http_inf_client_put_raw(url, len, p_inf) \
    http_inf_client_put_raw_field(url, len, p_inf, NULL, NULL)

/**
 * @brief This API is used to PUT raw HTTP request/response, extra header is supported
 *
 * @param[in] url URL of HTTP request
 * @param[in] len Max length of HTTP response data
 * @param[in] p_inf Pointer to HTTP_INF_H_S
