...
/**
 * @brief tuya Device OS 本地数据采集
 *
 * @param[in] dt: 数据类型，dt=0，上传所有数据
 * @param[in] json_buff: 组装日志的json字符串缓存
 * @param[in] json_len: 缓存大小
 *
 * @return int: 0成功，非0，请参照tuya error code描述文档
 */
OPERATE_RET tuya_devos_get_devlog(TY_DEVOS_DATA_TYPE dt, CHAR_T *json_buff, UINT_T json_len);
