...
extern struct server *g_net_video_rec;
static int fv_video_rec_start(void)
{
    ...
    printf("\n fv_video_rec_start>>>>>>s_info.width = %d, s_info.height = %d...>>>>>>>>>>>>>>>>>>\n ", s_info.width, s_info.height);
...
#endif // CONFIG_UVC_VIDEO2_ENABLE
...
#define NET_ISC_SBUF_SIZE     	(CONFIG_VIDEO_IMAGE_W*32*3/2)/*720P内部ISC BUFF采用内部ram*/
    ...
    err = server_request(__this_strm->video_rec0, VIDEO_REQ_SET_PACKET_CALLBACK, &req);
    if (err) {
        return err;
    }
