...
static int lcd_480x272_xxxx_init(struct lcd_board_cfg *bd_cfg)
{
    printf("lcd 480x272_xxxx 8bits init ...\n");

    return 0;
}

static int lcd_480x272_xxxx_check_id(struct lcd_board_cfg *bd_cfg)
{
    // TODO: check id
...
REGISTER_IMD_DEVICE_BEGIN(lcd_480x272_xxxx_dev) = {
    .info = {
#ifdef USE_TEST_MODE
        .test_mode 	     = true,
#else
...
REGISTER_LCD_DEVICE_DRIVE(lcd_dev)  = {
    ...
    .bl_ctrl         = lcd_480x272_xxxx_backctrl,
    .check           = lcd_480x272_xxxx_check_id,
    .send_init_code  = NULL,
};
