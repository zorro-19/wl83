...
#define CONFIG_FAT_FLASH_SIZE __EXTERN_FLASH_SIZE__

// 如果需要使用16M以上的flash需要修改download.bat文件与之对应
#if (CONFIG_FAT_FLASH_SIZE > 0x800000)
...
static void virfat_flash_flush(void *fd);
...
static void fat_copy(void *pDest, void *pSrc, u32 copyLen)
{
    while (copyLen--) {
...
static unsigned char bpb_data[512] = {
...
static int virfat_flash_mount(virfat_flash_t *disk_handler, u8 *buf)
{
    u32 fatsize;
    u32 maxclust;
    u32 sects_fat;
...
static int virfat_flash_read(void *fd, void *buf, u32 addr_sec, u32 len)
{
    ...
    if (align_addr == virfat_addr) {
        virfat_flash_flush(fd);
    }
#endif

    return virfat_read(fd, buf, real_addrsec * 512, len);
}

static int virfat_flash_write(void *fd, void *buf, u32 addr_sec, u32 len)
{
...
u32 virfat_flash_read_watch(void *buf, u32 addr_sec, u32 len)
{
    ...
    if (align_addr == virfat_addr) {
        virfat_flash_flush(g_fd);
    }
#endif

    return virfat_read(g_fd, buf, real_addrsec, len);
}
...
u32 virfat_flash_write_watch(void *buf, u32 addr_sec, u32 len)
{
    ...
    return virfat_write(g_fd, buf, real_addrsec, len);
}
...
u32 virfat_flash_erase_watch(int cmd, u32 arg)
{
    ...
    return virfat_ioctl(g_fd, cmd, arg);
}

u32 virfat_flash_get_real_capacity(void *fd) //获取实际flash容量
{
...
void virfat_flash_backups_fat_dir_data(void)
{
    u8 *buf = zalloc(4096);
    if (!buf) {
        return;
...
void virfat_flash_resume_fat_dir_data(void)
{
    u8 *buf = zalloc(4096);
    if (!buf) {
        return;
...
static void virfat_flash_init(void *fd)
{
    u32 zero = 0;
    virfat_flash_mount(&virfat_flash, bpb_data);
    int real_capacity = virfat_flash_get_real_capacity(fd);
    ...
    if (virfat_flash.capacity != real_capacity) {
        if (real_capacity >= 32 * 1024 * 1024 / 512) {
            fat_copy(&bpb_data[BPB_TotSec16], &zero, 2);
...
static int flash_virfat_open(const char *name, struct device **device, void *arg)
{
    *device = &virfat_flash_dev;
    void *fd = dev_open((const char *)&virfat_flash.re_devname, arg);
    ASSERT(fd);
    virfat_flash_init(fd);
    (*device)->private_data = fd;
    g_fd = fd;

    return 0;
}

static int flash_virfat_read(struct device *device, void *buf, u32 len, u32 addr)
{
    void *fd = device->private_data;
    /* y_printf(">>>[test]:r；addr = %d ,len = %d\n", addr, len); */
    void *_buf = buf;
    u32 _len = len;
    u32 _lba = addr;
    while (_len) {
        virfat_flash_read(fd, _buf, _lba, 512);
        _len -= 1;
        _lba += 1;
        _buf += 512;
    }
...
static int flash_virfat_write(struct device *device, void *buf, u32 len, u32 addr)
{
    ...
    u32 _lba = addr;
    while (_len) {
        virfat_flash_write(fd, _buf, _lba, 512);
        _len -= 1;
        _lba += 1;
        _buf += 512;
    }
...
static void virfat_flash_flush(void *fd)
{
    ...
    if (virfat_addr) {
        virfat_ioctl(fd, IOCTL_ERASE_SECTOR, virfat_addr);
        if (virfat_write(fd, buf_write, virfat_addr, 4096) != 4096) {
...
static int flash_virfat_ioctrl(struct device *device, u32 cmd, u32 arg)
{
    ...
    case IOCTL_GET_CAPACITY:
        *((u32 *)arg) = virfat_flash.capacity * 512;
        break;
...
static int _private_sfc_read(struct device *device, void *buf, u32 addr, u32 len)
{
    ...
    MEMCPY_FLASH(buf, (void *)cpu_addr, len);
    /* put_buf(buf, len); */
    return len;
}

static int _private_sfc_write(struct device *device, void *buf, u32 addr, u32 len)
{
    FLASH_PRIVATE_INFO *flash_private_info = (FLASH_PRIVATE_INFO *)device->private_data;
    if (flash_private_info->private_len < (addr + len)) {
        return -1;
    ...
    return norflash_write(NULL, buf, len, addr);
}

static int _private_sfc_ioctl(struct device *device, u32 cmd, u32 arg)
{
    FLASH_PRIVATE_INFO *flash_private_info = (FLASH_PRIVATE_INFO *)device->private_data;
...
static int private_sfc_read(struct device *device, void *buf, u32 len, u32 addr)
{
    /* y_printf(">>>[test]:r；addr = %d ,len = %d\n", addr, len); */
    return _private_sfc_read(device, buf, addr, len);
}

static int private_sfc_write(struct device *device, void *buf, u32 len, u32 addr)
{
    /* r_printf(">>>[test]:w；addr = %d ,len = %d\n", addr, len); */
    return _private_sfc_write(device, buf, addr, len);
}
