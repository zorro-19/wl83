...
void fm_IIC_write(u8 w_chip_id, u8 register_address, u8 *buf, u32 data_len)
{
    ...
    while (data_len--) {
        if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE, *buf++)) {
            printf("iic write err!!! line : %d \n", __LINE__);
            goto exit;
        }
    }
...
u8 fm_IIC_readn(u8 r_chip_id, u8 register_address, u8 *buf, u8 data_len)
{
    ...
    if (0xff != register_address) {
        if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE_WITH_START_BIT, (r_chip_id & 0x01) ? (r_chip_id - 1) : (r_chip_id))) {
    ...
    while (data_len-- > 1) {
        dev_ioctl(iic, IOCTL_IIC_RX_BYTE_WITH_ACK, (u32)buf);
        ++buf;
        ++read_len;
    }

    dev_ioctl(iic, IOCTL_IIC_RX_BYTE_WITH_STOP_BIT, (u32)buf);
