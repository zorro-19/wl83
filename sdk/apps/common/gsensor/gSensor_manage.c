...
unsigned char gravity_sensor_command(unsigned char w_chip_id, unsigned char register_address, unsigned char function_command)
{
    ...
    if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE_WITH_START_BIT, w_chip_id)) {
        ret = 0;
        puts("\n gsen iic wr err 0\n");
        goto __gcend;
    }
    ...
    if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE, register_address)) {
        ret = 0;
        puts("\n gsen iic wr err 1\n");
        goto __gcend;
    }
    ...
    if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE_WITH_STOP_BIT, function_command)) {
        ret = 0;
        puts("\n gsen iic wr err 2\n");
        goto __gcend;
    }
...
unsigned char _gravity_sensor_get_data(unsigned char w_chip_id, unsigned char r_chip_id, unsigned char register_address)
{
    ...
    delay(iic_delay);
//    if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE_WITH_STOP_BIT, register_address)) {
    if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE, register_address)) {
        puts("\n gsen iic rd err 1\n");
        regDat = 0;
        goto __gdend;
    ...
    delay(iic_delay);
    if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE_WITH_START_BIT, r_chip_id)) {
        regDat = 0;
        puts("\n gsen iic rd err 2\n");
        goto __gdend;
    }
...
void sys_power_set_port_wakeup(char *wkup_n, int enable)
{
    if (!strcmp(wkup_n, "wkup_gsen")) {
        if (enable) {
            p33_io_wakeup_enable(gSensor_wakeup_io, enable);
        }
    }
}

u8 sys_power_get_wakeup_reason()
{
    if (is_gpio_wakeup(gSensor_wakeup_io) == 1) {
        return 1;
    }
    return 0;
}
