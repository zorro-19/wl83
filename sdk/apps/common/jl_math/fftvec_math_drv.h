...
#define  VECX_S32           (2<<6)	//x输入位宽32bit
#define  VECX_S16           (1<<6)	//x输入位宽16bit
#define  VECX_S8            (0<<6)	//x输入位宽8bit

#define  VECY_S32           (2<<8)	//y输入位宽32bit
#define  VECY_S16           (1<<8)	//y输入位宽16bit
#define  VECY_S8            (0<<8)	//y输入位宽8bit

#define  VECZ_S64           (3<<10)	//z输入输出位宽64bit(只用于复数平方运算)
#define  VECZ_S32           (2<<10)	//z输入输出位宽32bit
#define  VECZ_S16           (1<<10)	//z输入输出位宽16bit
#define  VECZ_S8            (0<<10)	//z输入输出位宽8bit


#define  VEC_MIN            (1<<20)	//实数与CONST比较取最小值
#define  VEC_MAX            (1<<19)	//实数与CONST比较取最大值
#define  VEC_CPX_QDT        (1<<18)	//复数平方运算
#define  VEC_CONST_MUL      (1<<17)	//常数乘矢量运算
#define  VEC_RES_NEG_ZERO   (1<<16)	//当运算结果为负数时, 将输出结果设为0
#define  VEC_MAC32          (1<<15)	//复数或实数乘加运算(只支持32位运算)
#define  VEC_QDT            (1<<14)	//平方运算
#define  VECY_CONJ          (1<<13)	//复数乘时, 输入y_data先做共轭运算(Y_CONJ与X_CONJ不能同时配置)
#define  VECX_CONJ          (1<<12)	//复数乘时, 输入x_data先做共轭运算(Y_CONJ与X_CONJ不能同时配置)

#define  VEC_ACC            (1<<5)	//矢量点乘累加运算
#define  VEC_SUB            (1<<4)	//矢量减法运算

#define  VEC_REAL_ADDSUB_MODE            (0<<0)	//实数加减法运算
#define  VEC_REAL_MUL_MODE               (1<<0)	//实数乘法运算
#define  VEC_CPX_MUL_MODE                (2<<0)	//复数乘法运算
#define  VEC_QDT_MODE                    (3<<0)	//平方运算
...
#define  VEC_YPTR_NOCPY					 (1<<28)		//是否需要复制输入YPTR, just for mark,
