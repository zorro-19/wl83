...
//按键驱动扫描参数列表
static struct key_driver_para irkey_scan_param = {
...
static u8 ir_key_timer_index;

//获取ir按键值
static u8 ir_get_key_value(void)
{
    u8 tkey = 0xff;
    tkey = get_irflt_value(ir_key_timer_index);
    if (tkey == 0xff) {
        return tkey;
    }
    tkey = IRTabFF00[tkey];
...
static int irkey_init(void)
{
    const struct irkey_platform_data *irkey_data = get_irkey_platform_data();
    if (!irkey_data) {
        return -EINVAL;
    }

    if (irkey_data->port != -1) {
        ir_input_io_sel(irkey_data->port, irkey_data->timer_index);
    }

    ir_key_timer_index = irkey_data->timer_index;

    irflt_config(irkey_data->timer_index);

    ir_timeout_set(irkey_data->timer_index);

    return 0;
}

REGISTER_KEY_DRIVER_OPS(irkey) = {
