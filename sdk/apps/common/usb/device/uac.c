...
#define UAC_ISO_SLAVE_TEST 0

#if UAC_ISO_SLAVE_TEST
#define	EP_ISO_SLAVE_TEST 1024
#endif
...
#if UAC_ISO_SLAVE_TEST
    spk_frame_len = EP_ISO_SLAVE_TEST;
#endif
...
static void open_spk(struct usb_device_t *usb_device)
{
    ...
    u8 *ep_buffer = uac_info[usb_id]->spk_dma_buffer;
    uac_speaker_stream_open(usb_id, SPK_AUDIO_RATE, SPK_CHANNEL, SPK_AUDIO_RES);
...
static void close_spk(struct usb_device_t *usb_device)
{
    ...
    uac_speaker_stream_close(usb_id, 0);
}
...
#if UAC_ISO_SLAVE_TEST
    mic_frame_len = EP_ISO_SLAVE_TEST;
#endif
...
static void open_mic(struct usb_device_t *usb_device)
{
    ...
    uac_mic_stream_open(usb_id, mic_samplingfrequency, mic_frame_len, MIC_CHANNEL, MIC_AUDIO_RES);
...
u32 uac_audio_desc_config(const usb_dev usb_id, u8 *ptr, u32 *cur_itf_num)
{
    ...
    log_info("audio control interface num:%d", *cur_itf_num);
    if (usb_set_interface_hander(usb_id, *cur_itf_num, audio_ac_itf_handler) != *cur_itf_num) {
        ASSERT(0, "uac spk set interface_hander fail");
    }
    ...
    (*cur_itf_num) ++;
    log_info("speaker stream interface num:%d", *cur_itf_num);
    if (usb_set_interface_hander(usb_id, *cur_itf_num, spk_as_itf_hander) != *cur_itf_num) {
        ASSERT(0, "uac spk set interface_hander fail");
    }

    (*cur_itf_num) ++;
    log_info("mic stream interface num:%d", *cur_itf_num);
    if (usb_set_interface_hander(usb_id, *cur_itf_num, mic_as_itf_hander) != *cur_itf_num) {
        ASSERT(0, "uac mic set interface_hander fail");
    }
