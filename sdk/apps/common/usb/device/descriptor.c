...
#else
    extern __attribute__((weak)) u8 *get_norflash_uuid(u8 id);
    u8 flash_id[16] = {0};
    int i;
    u8 bcd;
    if (get_norflash_uuid && get_norflash_uuid(0)) {
        ptr[0] = 0x22;
        ptr[1] = 0x03;
        memset(&ptr[2], 0, 0x20);
        memcpy(flash_id, get_norflash_uuid(0), 16);
...
const u8 *usb_get_config_desc(void)
{
