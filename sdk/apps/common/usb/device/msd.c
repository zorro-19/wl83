...
#include "debug.h"
 ...
 // *INDENT-OFF*
#define USB_MSD_BULK_DEV_USE_ASYNC			0           //默认关闭msd异步访问sdio功能，要想使用必须先卸载文件系统
void wdt_clear(void);
...
static void msd_endpoint_init(struct usb_device_t *usb_device, u32 itf)
{
    ...
    usb_g_ep_config(usb_id, MSD_BULK_EP_IN | USB_DIR_IN, USB_ENDPOINT_XFER_BULK, 0, msd_handle[usb_id]->ep_out_dmabuffer, usb_device->bSpeed == USB_SPEED_HIGH ? MAXP_SIZE_BULKIN_HS : MAXP_SIZE_BULKIN_FS);
    usb_g_ep_config(usb_id, MSD_BULK_EP_OUT, USB_ENDPOINT_XFER_BULK, 1, msd_handle[usb_id]->ep_out_dmabuffer, usb_device->bSpeed == USB_SPEED_HIGH ? MAXP_SIZE_BULKOUT_HS : MAXP_SIZE_BULKOUT_FS);
...
static void inquiry(const struct usb_device_t *usb_device)
{
    ...
    u32 cd_scsiinquirydata[sizeof(CD_SCSIInquiryData) / 4 + 4]; //usb dma 不能访问code区域
    if (cur_lun == 0) {
        len = min(msd_handle[usb_id]->csw.uCSWDataResidue, sizeof(CD_SCSIInquiryData));
        memcpy(cd_scsiinquirydata, CD_SCSIInquiryData, sizeof(CD_SCSIInquiryData));
        err = msd_mcu2usb(usb_device, (u8 *)cd_scsiinquirydata, len);
    } else
#endif
    {
...
{
    const usb_dev usb_id = usb_device2id(usb_device);
    u32 err = msd_mcu2usb(usb_device, (u8 *)scsi_mode_sense, sizeof(scsi_mode_sense));
    if (err == 0) {
        log_error("write usb err %d", __LINE__);
    }
    msd_handle[usb_id]->csw.uCSWDataResidue = 0;
...
static void msd_read_cdrom_top(const struct usb_device_t *usb_device)
{
    /* printf("%s() %d\n", __func__, len); */
    const usb_dev usb_id = usb_device2id(usb_device);
    u32 len = min(msd_handle[usb_id]->csw.uCSWDataResidue, sizeof(cdrom_top_info));
...
static void write_10(const struct usb_device_t *usb_device)
{
    ...
    while (lba_num) {
        wdt_clear();
        u16 num = lba_num > MSD_BLOCK_SIZE ? MSD_BLOCK_SIZE : lba_num;
        if (msd_handle[usb_id]->csw.uCSWDataResidue == 0) {
            msd_handle[usb_id]->csw.bCSWStatus = 0x1;
            break;
        }
    ...
    if (msd_handle[usb_id]) {
        for (i = 0; i < __get_max_msd_dev(); i++) {
            if (msd_handle[usb_id]->info.dev_handle[i]) {
                dev_close(msd_handle[usb_id]->info.dev_handle[i]);
                msd_handle[usb_id]->info.dev_handle[i] = NULL;
...
u32 msd_register(const usb_dev usb_id)
{
    if (msd_handle[usb_id] == NULL) {
        ...
        msd_handle[usb_id]->ep_out_dmabuffer = usb_alloc_ep_dmabuffer(usb_id, MSD_BULK_EP_OUT, MAXP_SIZE_BULKIN_FS + MAXP_SIZE_BULKOUT_FS);
#else
        msd_handle[usb_id]->ep_out_dmabuffer = usb_alloc_ep_dmabuffer(usb_id, MSD_BULK_EP_OUT, usb_id ? MAXP_SIZE_BULKIN_HS + MAXP_SIZE_BULKOUT_HS : MAXP_SIZE_BULKIN_FS + MAXP_SIZE_BULKOUT_FS);
#endif
    }
    return 0;
}
