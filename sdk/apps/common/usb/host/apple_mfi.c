    ...
    if (apple_mfi_chip_online_lib()) {
        ...
        if ((device_desc.idVendor == 0x05AC) && ((device_desc.idProduct & 0xff00) == 0x1200)) {
            ...
            for (cnt = 0; cnt <= 3; cnt++) {
                ...
                get_config_descriptor_add_value_l(host_dev, &cfg_desc, USB_DT_CONFIG_SIZE, cnt);
                /* check_usb_mount(ret); */
                ...
                get_config_descriptor_add_value_l(host_dev, desc_buf, cfg_desc.wTotalLength, cnt);
                /* check_usb_mount(ret); */
            ...
            set_configuration_add_value(host_dev, bDescriptorNum);
            /* check_usb_mount(ret); */
