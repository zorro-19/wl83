...
#if 0
void lv_rgb332_to_index8(uint8_t *src, int src_size, uint8_t *dest)
{
    lv_color32_t ret = {
        .ch.alpha = 0xff,
        .ch.red = 0,
    ...
    lv_color8_t *color_8 = (lv_color8_t *)src;
    lv_color32_t *palette_p = (lv_color32_t *)dest;

    uint16_t palette_size = 256;
    uint32_t i, j;

    /* memset(dest, 0x00, 1024); //clear palette */

    dest += 1024;

    for (i = 0; i < src_size; i++) {
        LV_COLOR_SET_R32(ret, LV_COLOR_GET_R(color_8[i]) * 36);
        LV_COLOR_SET_G32(ret, LV_COLOR_GET_G(color_8[i]) * 36);
        LV_COLOR_SET_B32(ret, LV_COLOR_GET_B(color_8[i]) * 85);
        //LV_COLOR_SET_A32(ret, 0xFF);
        for (j = 0; j < palette_size; j++) {
            if (palette_p[j].full == ret.full) {
                *dest++ = j;
                break;
            }
            if (j > 0 && palette_p[j].full == 0) {
                palette_p[j].full = ret.full;
                *dest++ = j;
                break;
            }
        }
        if (j >= palette_size) {
            printf("palette overflow!\n");
        }
    }
}
#endif
