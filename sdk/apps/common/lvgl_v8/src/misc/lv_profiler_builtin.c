...
#define profiler_ctx profiler_context //LV_GLOBAL_DEFAULT()->profiler_context
...
void lv_profiler_builtin_config_init(lv_profiler_builtin_config_t *config)
{
    ...
    lv_memset_00(config, sizeof(lv_profiler_builtin_config_t));
...
void lv_profiler_builtin_init(const lv_profiler_builtin_config_t *config)
{
    ...
    lv_memset_00(&profiler_ctx, sizeof(profiler_ctx));
    profiler_ctx.item_arr = lv_mem_alloc(num * sizeof(lv_profiler_builtin_item_t));
...
void lv_profiler_builtin_uninit(void)
{
    ...
    lv_mem_free(profiler_ctx.item_arr);
    lv_memset_00(&profiler_ctx, sizeof(profiler_ctx));
}
