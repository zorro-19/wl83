...
void lv_img_set_src(lv_obj_t *obj, const void *src)
{
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) {
        ...
        img->src = src;
    } else if (src_type == LV_IMG_SRC_FILE || src_type == LV_IMG_SRC_SYMBOL || src_type == LV_IMG_SRC_BIN) {
        ...
        if (img->src != src) {
            ...
            if (img->src_type == LV_IMG_SRC_FILE || img->src_type == LV_IMG_SRC_SYMBOL || img->src_type == LV_IMG_SRC_BIN) {
                old_src = img->src;
            }
