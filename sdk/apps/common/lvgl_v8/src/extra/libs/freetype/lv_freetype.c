...
#if LV_USE_GPU_JL_GPU2P5D
static const lv_font_path_t *get_glyph_vertex_path_cb_cache(const lv_font_t *font_p, uint32_t letter)
{
    //TODO
    return NULL;
}
...
#if LV_USE_GPU_JL_GPU2P5D
    lv_font_jlgpu_privately_t *jlgpu_privately = (lv_font_jlgpu_privately_t *)lv_mem_alloc(sizeof(lv_font_jlgpu_privately_t));
    jlgpu_privately->is_support = false;   // 暂时未支持
    jlgpu_privately->get_glyph_vertex_path = get_glyph_vertex_path_cb_cache;
    font->user_data = jlgpu_privately;
#else
    font->user_data = NULL;
#endif
...
#if LV_USE_GPU_JL_GPU2P5D
static const lv_font_path_t *get_glyph_vertex_path_cb_nocache(const lv_font_t *font_p, uint32_t letter)
{
    //TODO
    return NULL;
}
...
static bool lv_ft_font_init_nocache(lv_ft_info_t *info)
{
    ...
    lv_font_jlgpu_privately_t *jlgpu_privately = (lv_font_jlgpu_privately_t *)lv_mem_alloc(sizeof(lv_font_jlgpu_privately_t));
    jlgpu_privately->is_support = false;   // 暂时未支持
    jlgpu_privately->get_glyph_vertex_path = get_glyph_vertex_path_cb_nocache;
    font->user_data = jlgpu_privately;
#else
    font->user_data = NULL;
#endif
