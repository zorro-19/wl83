...
{
    lv_memset_00(subject, sizeof(lv_subject_t));
...
void lv_subject_copy_string(lv_subject_t *subject, const char *buf)
{
    ...
    if (subject->prev_value.pointer) {
        strncpy((char *)subject->prev_value.pointer, subject->value.pointer, subject->size - 1);
    }

    strncpy((char *)subject->value.pointer, buf, subject->size - 1);
...
void lv_observer_remove(lv_observer_t *observer)
{
    ...
    if (observer->auto_free_user_data) {
        lv_mem_free(observer->user_data);
    }
                                       ...
                                       int32_t ref_value, bool inv)
{
    ...
    flag_and_cond_t *p = lv_mem_alloc(sizeof(flag_and_cond_t));
    if (p == NULL) {
