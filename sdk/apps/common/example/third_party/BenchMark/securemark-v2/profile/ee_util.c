...
void
ee_hextotxt(uint8_t *p_addr, uint32_t len)
{
    uint32_t i;
    char     b;
    for (i = 0; i < len; ++i) {
...
ee_printmemline(uint8_t *p_addr, uint32_t len, char *p_user_header)
{
    ...
    th_printf(p_header);
    th_printf("0x");
    ee_hextotxt(p_addr, len);
    th_printf("\r\n");
}

uint32_t
ee_fix_endian32(uint32_t x)
{
    union checker {
        uint32_t dword;
        uint8_t byte[4];
    ...
    if (reference.byte[0] == 1) {
        /* little endian machine */
        return ((((x) & 0xff000000) >> 24) | (((x) & 0x00ff0000) >>  8)
                | (((x) & 0x0000ff00) <<  8) | (((x) & 0x000000ff) << 24));
    }
    /* big endian machine, make sure swap does nothing */
    return x;
}
