...
ee_status_t
th_ecdh_create(void **pp_context, ee_ecc_group_t group)
{
    int    ret;
...
th_ecdh_get_public_key(void *p_context, uint8_t *p_pub, uint32_t *p_publen)
{
    ...
    case ECC_SECP256R1:
    case ECC_SECP384R1:
        /* Force a keysize calculation */
        wc_ecc_export_x963(&(ctx->key.ecc), NULL, &publen);
        CHK1(wc_ecc_export_x963(&(ctx->key.ecc), p_pub, &publen));
        break;
    case ECC_X25519:
        /* Force a keysize calculation */
        wc_curve25519_export_public_ex(
            &(ctx->key.c25519), p_pub, &publen, EC25519_LITTLE_ENDIAN);
