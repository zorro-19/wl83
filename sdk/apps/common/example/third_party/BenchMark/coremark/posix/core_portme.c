...
#define GETMYTIME(_t)              (*_t = timer_get_ms())
...
*/
#if USE_PTHREAD
static unsigned int thread_fork_cnt;
static unsigned char coremark_iterate0_task_run_end_flag, coremark_iterate1_task_run_end_flag;
static void coremark_iterate_task(core_results *res)
{
    iterate(res);
    res->port.thread_kill_flag = 1;
    while (1) {
        msleep(60000);
    }
}
...
{
    res->port.thread_kill_flag = 0;

    if (thread_fork_cnt == 0) {
        os_task_create(coremark_iterate_task, res, 30, 1000, 0, "coremark_iterate0");
    } else if (thread_fork_cnt == 1) {
        os_task_create(coremark_iterate_task, res, 30, 1000, 0, "coremark_iterate1");
    } else {
        puts("[core_start_parallel] Please add more thread here\r\n");
    }
...
{
    volatile int *volatile p_thread_kill_flag = &res->port.thread_kill_flag;
    while (*p_thread_kill_flag == 0) {
