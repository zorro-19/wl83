...
void lv_bidi_process(const char *str_in, char *str_out, lv_base_dir_t base_dir)
{
    if (base_dir == LV_BASE_DIR_AUTO) {
        base_dir = lv_bidi_detect_base_dir(str_in);
    }
                               ...
                               uint16_t *pos_conv_out, uint16_t pos_conv_len)
{
    ...
    while (rd < len) {
        ...
        if (dir == LV_BASE_DIR_NEUTRAL) {
            dir = bracket_process(&ctx, str_in, rd, len, letter, base_dir);
        } else if (dir != LV_BASE_DIR_WEAK) {
            break;
        }
    }
                                  ...
                                  uint16_t   *pos_conv_len)
{
    ...
    if (dir == LV_BASE_DIR_NEUTRAL) {
        dir = bracket_process(ctx, txt, 0, max_len, letter, base_dir);
    }
    ...
    while (dir == LV_BASE_DIR_NEUTRAL || dir == LV_BASE_DIR_WEAK) {
        ...
        dir = lv_bidi_get_letter_dir(letter);
        if (dir == LV_BASE_DIR_NEUTRAL) {
            dir = bracket_process(ctx, txt, i, max_len, letter, base_dir);
        }
    ...
    while (i_prev < max_len && txt[i] != '\0' && txt[i] != '\n' && txt[i] != '\r') {
        ...
        if (next_dir == LV_BASE_DIR_NEUTRAL) {
            next_dir = bracket_process(ctx, txt, i, max_len, letter, base_dir);
        }
                                     ...
                                     lv_base_dir_t base_dir)
{
    ...
    for (i = 0; bracket_left[i] != '\0'; i++) {
        if (bracket_left[i] == letter) {
            ...
            txt_i = next_pos;
            if (txt_i) {
                lv_text_encoded_prev(txt, &txt_i);
            }
            while (txt_i > 0) {
    ...
    if (bracket_left[i] != '\0') {
        ...
        if (bracket_dir == LV_BASE_DIR_NEUTRAL || ctx->br_stack_p == LV_BIDI_BRACKET_DEPTH) {
            return LV_BASE_DIR_NEUTRAL;
        }
