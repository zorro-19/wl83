...
void lv_draw_sw_fill(lv_draw_unit_t *draw_unit, lv_draw_fill_dsc_t *dsc, const lv_area_t *coords)
{
    ...
    if (!lv_area_intersect(&clipped_coords, &bg_coords, draw_unit->clip_area)) {
        return;
    }
        ...
        if (top_y >= clipped_coords.y1) {
            ...
            if (preblend) {
                ...
                for (i = 0; i < clipped_w; i++) {
                    if (grad_opa_map[i] < LV_OPA_MAX) {
                        mask_buf[i] = (mask_buf[i] * grad_opa_map[i]) >> 8;
                    }
                }
                blend_dsc.mask_res = LV_DRAW_SW_MASK_RES_CHANGED;
            }
        ...
        for (h = h_start; h <= h_end; h++) {
            ...
            case LV_GRAD_DIR_VER:
                ...
                if (opa >= LV_OPA_MAX) {
                    ...
                    blend_dsc.opa = LV_OPA_MIX2(grad->opa_map[h - bg_coords.y1], opa);
                }
                break;
#if LV_USE_DRAW_SW_COMPLEX_GRADIENTS
            case LV_GRAD_DIR_LINEAR:
