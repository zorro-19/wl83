                          ...
                          const lv_area_t *img_coords, const lv_area_t *clipped_img_area)
{
    ...
    if (!transformed && !masked && cf == LV_COLOR_FORMAT_A8) {
        ...
        if (!lv_area_intersect(&clipped_coords, img_coords, draw_unit->clip_area)) {
            return;
        }
        ...
        if (decoder_res == LV_RESULT_OK && mask_decoder_dsc.decoded) {
            if (mask_decoder_dsc.decoded->header.cf == LV_COLOR_FORMAT_A8 ||
                ...
                if (lv_area_get_width(&draw_dsc->image_area) < 0) {
                    image_area = img_coords;
                } else {
                    image_area = &draw_dsc->image_area;
                }
    ...
    /* check whether it is possible to accelerate the operation in synchronous mode */
        ...
        if (transformed) {
            ...
            } else if (cf_final == LV_COLOR_FORMAT_L8) {
                cf_final = LV_COLOR_FORMAT_AL88;
            }
        }
