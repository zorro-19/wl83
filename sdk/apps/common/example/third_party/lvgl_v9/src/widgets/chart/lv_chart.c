...
lv_chart_series_t *lv_chart_get_series_next(const lv_obj_t *obj, const lv_chart_series_t *ser)
{
    ...
    lv_chart_t *chart  = (lv_chart_t *)obj;
    if (ser == NULL) {
        return lv_ll_get_head(&chart->series_ll);
    } else {
        return lv_ll_get_next(&chart->series_ll, ser);
    }
}
...
{
    lv_area_t clip_area;
    if (lv_area_intersect(&clip_area, &obj->coords, &layer->_clip_area) == false) {
        return;
    }
...
static void draw_cursors(lv_obj_t *obj, lv_layer_t *layer)
{
    ...
    lv_chart_t *chart  = (lv_chart_t *)obj;
    if (lv_ll_is_empty(&chart->cursor_ll)) {
        return;
    }

    lv_area_t clip_area;
    if (!lv_area_intersect(&clip_area, &layer->_clip_area, &obj->coords)) {
        return;
    }
