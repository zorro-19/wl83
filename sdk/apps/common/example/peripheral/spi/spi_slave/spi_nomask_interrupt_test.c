...
#ifdef USE_SPI_NOMASK_INTERRUPT_TEST_DEMO
...
static void spi_nomask_interrupt_test_task(void *arg)
{
    void *spi_hdl = NULL;
    int ret;
...
static int c_main_nomask_interrupt(void)
{
    os_task_create(spi_nomask_interrupt_test_task, NULL, 8, 1000, 0, "spi_nomask_interrupt_test_task");
    return 0;
}

late_initcall(c_main_nomask_interrupt);

#endif // USE_SPI_NOMASK_INTERRUPT_TEST_DEMO
