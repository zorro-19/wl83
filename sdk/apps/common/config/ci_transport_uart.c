...
static int ci_dev_open(void)
{
    ...
    dev_ioctl(__this->uart_dev, IOCTL_UART_SET_CIRCULAR_BUFF_ADDR, (int)uart_circlebuf);

    parm = sizeof(uart_circlebuf);

    dev_ioctl(__this->uart_dev, IOCTL_UART_SET_CIRCULAR_BUFF_LENTH, (int)parm);

#if 0 // 是否设置为 接收完指定长度数据, spec_uart_recv才出来
    parm = 1;
    dev_ioctl(__this->uart_dev, IOCTL_UART_SET_RECV_ALL, (int)parm);
#endif

#if 1 // 是否设置为阻塞方式读
    parm = 1;
    dev_ioctl(__this->uart_dev, IOCTL_UART_SET_RECV_BLOCK, (int)parm);
#endif

    parm = 1000;
    dev_ioctl(__this->uart_dev, IOCTL_UART_SET_RECV_TIMEOUT, (u32)parm);

    __this->run_flag = 1;

    thread_fork("ci_transport_task", 11, 256 * 3, 0, 0, ci_transport_task, NULL);

    dev_ioctl(__this->uart_dev, IOCTL_UART_START, (int)0);
