...
static struct cfg_tool_event spp_packet;
...
void all_assemble_package_send_to_pc(u8 id, u8 sq, u8 *buf, u32 len)
{
    ...
    cdc_write_data(s_usb_id, send_buf, len + 8);
...
{
    sprintf((char *)out, "%02x%02x-%02x%02x%02x%02x", buf[5], buf[4], buf[3], buf[2], buf[1], buf[0]);
}
...
static int app_cfg_tool_event_handler(struct cfg_tool_event *cfg_tool_dev)
{
    ...
    case ONLINE_SUB_OP_QUERY_BASIC_INFO:
        ...
        log_info("name = %s", __this->s_basic_info.sdkName);
