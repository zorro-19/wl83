/******************************************************************************************
 *  File Name:  sc1336_mipi_awb.c
 *  Created on: 2024/7/11
 *  Author:     xzpin
 ******************************************************************************************/

#include "asm/isp_alg.h"
#include "sc1336_mipi_awb.h"
...
static void sc1336_awb_post_process(u32 prob, u32 ev, u32 zone_type, u16 *r_gain, u16 *b_gain, void *data)
{
    //printf("%d,%d,%d,%d,%d\n", prob, ev, zone_type, *r_gain, *b_gain);
    *r_gain = ((u32) * r_gain) * 100 / 100;
    *b_gain = ((u32) * b_gain) * 100 / 100;

    if (prob <= 0 && zone_type < 0 && ev > (1 << 16) * 16) {
...
static isp_awb_ct2_params_t awb_ct2_params = {
    .awb_post_process = sc1336_awb_post_process,
...
static isp_awb_params_t sc1336_mipi_awb_params = {
...
void *sc1336_mipi_get_awb_params()
{
    return (void *)&sc1336_mipi_awb_params;
}
