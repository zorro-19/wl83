...
extern u32 SC1346_mipi_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 SC1346_mipi_set_shutter(isp_ae_shutter_t *shutter);
extern void  *SC1346_mipi_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz
const static u32 SC1346_mipi_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 SC1346_mipi_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
...
const static u32 SC1346_mipi_ae_50hz_60fps[][AE_CURVE_INFO_MAX] = {

    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    10000,       200,    AE_INTERP_GAIN},
    {6826,     10000,       400,    AE_INTERP_GAIN},
    {3413,     10000,       800,    AE_INTERP_GAIN},
    {1707,     10000,      1600,    AE_INTERP_GAIN},
    {853,      10000,      3200,    AE_INTERP_GAIN},
    {427,      10000,      6400,    AE_INTERP_GAIN},
    {213,      10000,     12800,    AE_INTERP_GAIN},
    {106,      10000,     4 * 6400,    AE_INTERP_GAIN},
    {53,       10000,     4 * 12800,    AE_INTERP_GAIN},
    {26,       10000,     4 * 25600,    AE_INTERP_GAIN},
    {13,       10000,     4 * 51200,    AE_INTERP_GAIN},
    {AE_CURVE_END, 0,         0,              0}
};
const static u32 SC1346_mipi_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 SC1346_mipi_ae_50hz_variable_fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
...
static isp_ae_params_t  SC1346_mipi_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = SC1346_mipi_calc_shutter,
        .set_shutter = SC1346_mipi_set_shutter,
        .get_ae_curve = SC1346_mipi_get_ae_curve,
    },
    .ae_step = 2, // 1 //PPK ADD ON 20170726
};


void SC1346_mipi_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = SC1346_mipi_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    SC1346_mipi_calc_shutter(&ae_shutter, time, gain);

    SC1346_mipi_set_shutter(&ae_shutter);
}

void   *SC1346_mipi_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = (void *)&SC1346_mipi_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            ...
            curve = (void *)&SC1346_mipi_ae_50hz_30fps;
        } else if (fps == 60) {
            curve = (void *)&SC1346_mipi_ae_50hz_60fps;
        }
        ...
        *max_ev = AE_MAX_EV;
        ...
        curve = (void *)&SC1346_mipi_ae_60hz_30fps;
...
void *SC1346_mipi_get_ae_params()
{
    return (void *)&SC1346_mipi_ae_params;
}
