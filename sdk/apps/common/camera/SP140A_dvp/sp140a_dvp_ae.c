...
#include "sp140a_dvp_ae.h"
#include "sp140a_dvp.h"

#include "asm/isp_alg.h"


extern u32 sp140a_dvp_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 sp140a_dvp_set_shutter(isp_ae_shutter_t *shutter);
extern void  *sp140a_dvp_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz

const static u32 sp140a_dvp_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 sp140a_dvp_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    10000,       200,    AE_INTERP_GAIN},
    {6826,     20000,       200,    AE_INTERP_GAIN},
    {3413,     20000,       400,    AE_INTERP_GAIN},
    {1707,     30000,       533,    AE_INTERP_GAIN},
    {853,      30000,      1066,    AE_INTERP_GAIN},
    {427,      30000,      2132,    AE_INTERP_GAIN},
    {213,      30000,      4264,    AE_INTERP_GAIN}, // lv = 3.73
    {106,      30000,      8528,    AE_INTERP_GAIN},
    { 53,      30000,     17056,    AE_INTERP_GAIN},

    // {0,        30000,      12748,   AE_INTERP_GAIN},
    {AE_CURVE_END, 0,         0,              0}
};

const static u32 sp140a_dvp_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  //lv = 20
...
static isp_ae_params_t  sp140a_dvp_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = sp140a_dvp_calc_shutter,
        .set_shutter  = sp140a_dvp_set_shutter,
        .get_ae_curve = sp140a_dvp_get_ae_curve,
    },
};


void sp140a_dvp_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = sp140a_dvp_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    sp140a_dvp_calc_shutter(&ae_shutter, time, gain);

    sp140a_dvp_set_shutter(&ae_shutter);
}


void   *sp140a_dvp_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    const void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = sp140a_dvp_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            ...
            curve = (void *)sp140a_dvp_ae_50hz_30fps;
        }
    } else {
        *min_ev = AE_MIN_EV;
        *max_ev = AE_MAX_EV;

        curve = sp140a_dvp_ae_60hz_30fps;
...
void *sp140a_dvp_get_ae_params()
{
    return (void *)&sp140a_dvp_ae_params;
}
