...
extern u32 sc1245_dvp_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 sc1245_dvp_set_shutter(isp_ae_shutter_t *shutter, u8 isp_dev);
extern void  *sc1245_dvp_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz
const static u32 sc1245_dvp_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 sc1245_dvp_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
...
const static u32 sc1245_dvp_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 sc1245_dvp_ae_50hz_variable_fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
...
static isp_ae_params_t  sc1245_dvp_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = sc1245_dvp_calc_shutter,
        .set_shutter = sc1245_dvp_set_shutter,
        .get_ae_curve = sc1245_dvp_get_ae_curve,
    },
    .ae_step = 2, // 1 //PPK ADD ON 20170726
};


void sc1245_dvp_ae_ev_init(u8 isp_dev, u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = sc1245_dvp_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    sc1245_dvp_calc_shutter(&ae_shutter[isp_dev], time, gain);

    sc1245_dvp_set_shutter(&ae_shutter[isp_dev], isp_dev);
}

void *sc1245_dvp_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = (void *)&sc1245_dvp_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        curve = &sc1245_dvp_ae_50hz_variable_fps;
#else
        *min_ev = AE_MIN_EV;
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            ...
            curve = (void *)&sc1245_dvp_ae_50hz_30fps;
        }
        ...
        *max_ev = AE_MAX_EV;
        ...
        curve = (void *)&sc1245_dvp_ae_60hz_30fps;
...
void *sc1245_dvp_get_ae_params()
{
    return (void *)&sc1245_dvp_ae_params;
}
