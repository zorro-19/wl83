...
#if TCFG_BLE_MESH_ENABLE && (TCFG_SIG_MESH_MODEL_SELECT == SIG_MESH_PROVISIONER)
...
extern void mesh_cli_cmd_rsp_callback_register(void (*handler)(u32_t opcode));
extern int bt_mesh_pb_adv_open(const u8_t uuid[16], u16_t net_idx, u16_t addr, u8_t attention_duration);
extern void clear_rpl(void);
extern void bt_mesh_tx_reset(void);
extern void mesh_prov_giving_up_callback_register(void (*handler)(void));
...
static const u8_t prov_net_key[16] = {
    0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
    0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
};

static const u8_t prov_dev_key[16] = {
    0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
    0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
};

static const u8_t prov_app_key[16] = {
...
static u16 prov_giving_up_timer;
                                ...
                                struct net_buf_simple *buf)
{
    ...
    gpio_direction_output(onoff_state->led_gpio_pin,
                          onoff_state->current);
}
 ...
 */
static void node_configuration_check(void *opcode)
{
    log_info("mesh_configuration_check:0x%x", opcode);

    if (cfg_cli.op_pending != (u32_t)opcode) {
        log_info("check pass");
    } else {
        // 重发超过10次放弃配置节点
...
static void mesh_set_configuration_check_timer(u32_t opcode)
{
    ...
    sys_timeout_add((void *)opcode, node_configuration_check, 3500);
}
...
static void provisioner_node_configuration(void)
{
    ...
    if (curr_pair_node.config_step == 3) {
        ...
        if (prov_giving_up_timer) {
            sys_timer_del(prov_giving_up_timer);
            prov_giving_up_timer = 0;
        }
#endif
    }
}
...
static u8_t show_node(struct bt_mesh_cdb_node *node,
                      void *user_data)
{
    log_info("[node addr = 0x%x]", node->addr);
    log_info("uuid = %s", bt_hex(node->uuid, 6));
    log_info("net_idx = 0x%x", node->net_idx);
    log_info("num_elem = %d", node->num_elem);
    log_info("dev_key = %s", bt_hex(node->dev_key, 16));
...
static u8_t del_node(struct bt_mesh_cdb_node *node,
                     void *user_data)
{
    log_info("<Del> [node addr = 0x%x]", node->addr);
...
#if TCFG_WIFI_ENABLE
static void prov_giving_up_timer_cb(void *p)
{
    prov_giving_up_timer = 0;
...
#if TCFG_WIFI_ENABLE
        void set_rf_coexistence_mesh_config(u8 enable);
        set_rf_coexistence_mesh_config(0);

        if (!prov_giving_up_timer) {
            prov_giving_up_timer = sys_timeout_add(NULL, prov_giving_up_timer_cb, 15000);
        } else {
            sys_timer_modify(prov_giving_up_timer, 15000);
        }
#endif
...
static void prov_giving_up(void)
{
#if TCFG_WIFI_ENABLE
    void set_rf_coexistence_mesh_config(u8 enable);
    set_rf_coexistence_mesh_config(2);
...
void input_key_handler(u8 key_status, u8 key_number)
{
    ...
    if (key_status == KEY_EVENT_LONG && key_number == 2) {
        switch (key_number) {
        case 0:
            break;
        case 2:
...
{
    log_info("node_config_cmd_rsp_cb, opcode:0x%x", opcode);
    cfg_cli.op_pending = 0;
    cfg_cli.op_param = NULL;
...
static void mesh_init(void)
{
    ...
    if (err) {
        log_error("Initializing mesh failed (err %d)", err);
        return;
    }
...
void bt_ble_mesh_init(void)
{
    ...
    mesh_prov_giving_up_callback_register(prov_giving_up);
