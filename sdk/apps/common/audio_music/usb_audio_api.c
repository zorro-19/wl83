...
static int usb_audio_mic_set_vol(struct usb_slave_mic_handle *hdl, u8 volume)
{
    if (hdl && hdl->rec_dev) {
        ...
        return dev_ioctl(hdl->rec_dev, AUDIOC_SET_VOLUME, (BIT(CONFIG_UAC_MIC_ADC_CHANNEL) << 8) | volume);
#else
        return dev_ioctl(hdl->rec_dev, AUDIOC_SET_VOLUME, volume);
#endif
    }
...
static int usb_audio_mic_open(struct usb_slave_mic_handle *hdl,  const usb_dev usb_id, int value)
{
    ...
    if (!mic_hdl) {
        log_error("uac slave audio_open: err");
        return -EFAULT;
    }
    ...
    hdl->f.channel = value >> 28;
    hdl->f.sample_rate = value & 0xfffff;
    hdl->f.frame_len = frame_len * 3;

    log_debug("uac slave channel : %d", hdl->f.channel);
    log_debug("uac slave sample_rate : %d", hdl->f.sample_rate);
    log_debug("uac slave volume : %d", hdl->f.volume);
    log_debug("uac slave frame_len : %d", hdl->f.frame_len);
    ...
    if ((hdl->f.sample_rate == 8000 || hdl->f.sample_rate == 16000) && hdl->f.channel == 1) {
        ...
        if (hdl->f.aec_enable) {
            ...
            hdl->f.rec_linein_channel_bit_map = BIT(CONFIG_AISP_LINEIN_ADC_CHANNEL); //设置linein的回采通道
    ...
    if (err) {
        log_error("uac slave audio_set_fmt: err");
        goto __err;
    }

    err = dev_ioctl(mic_hdl, AUDIOC_STREAM_ON, (u32)&hdl->bindex);
    if (err) {
        log_error(" uac slave audio rec stream on err");
        goto __err;
    }
    ...
    log_info("uac slave audio_enc_start: suss");
...
int usb_audio_event_handler(struct device_event *event)
{
    ...
    return product_usb_audio_event_handler(event);
#else
    ...
    case USB_AUDIO_PLAY_CLOSE:
        log_info("USB_AUDIO_SET_PLAY_CLOSE");
...
static int usb_host_audio_mic_open(struct usb_host_mic_handle *hdl, u32 sample_rate, u32 frame_len, u8 channel)
{
    ...
    if (!mic_hdl) {
        log_error("uac host audio_open: err");
        return -EFAULT;
    }
    ...
    log_debug("uac host channel : %d", hdl->f.channel);
    log_debug("uac host sample_rate : %d", hdl->f.sample_rate);
    log_debug("uac host volume : %d", hdl->f.volume);
    log_debug("uac host frame_len : %d", hdl->f.frame_len);
    ...
    err = dev_ioctl(mic_hdl, AUDIOC_SET_FMT, (unsigned int)&hdl->f);
    if (err) {
        log_error("uac host audio_set_fmt: err");
        goto __err;
    }

    err = dev_ioctl(mic_hdl, AUDIOC_STREAM_ON, (u32)&hdl->bindex);
    if (err) {
        log_error(" uac host audio rec stream on err");
        goto __err;
    }

    hdl->rec_dev = mic_hdl;

    log_info("uac host audio_enc_start: suss");
