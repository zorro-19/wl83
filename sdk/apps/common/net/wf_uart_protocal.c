...
static void uart_dev_init(void *priv)
{
    ...
    dev_ioctl(g_hdl, IOCTL_UART_SET_CIRCULAR_BUFF_ADDR, (int)cbuf);
    dev_ioctl(g_hdl, IOCTL_UART_SET_CIRCULAR_BUFF_LENTH, sizeof(cbuf));
    dev_ioctl(g_hdl, IOCTL_UART_SET_RECV_BLOCK, 1);
    dev_ioctl(g_hdl, IOCTL_UART_START, 0);
    ...
    while (1) {
        ...
        if (len <= 0) {
            if (len == UART_CIRCULAR_BUFFER_WRITE_OVERLAY) {
                ...
                dev_ioctl(g_hdl, IOCTL_UART_FLUSH, 0);
