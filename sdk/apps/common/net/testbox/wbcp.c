...
static u8 assist_dev_init(void)
{
    ...
    dev_ioctl(uart_hdl, IOCTL_UART_SET_CIRCULAR_BUFF_LENTH, sizeof(uart_buf));
...
static u32 wifi_send_packet(char *pkg, int len, char mcs, char phymode)
{
    ...
    memcpy(wifi_get_payload_ptr(), pkg, len + 4);
    wifi_send_data(len + 4, i);
...
void wbcp_rx_frame_cb(void *rxwi, void *header, void *data, u32 len, void *priv)
{
