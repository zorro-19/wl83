...
extern void tvs_tts_player_close(void);
extern int tts_volume_change(void);
extern void set_default_length(void);	//进入配网，设置默认最大数据包长度
...
static void mix_dec_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case AUDIO_SERVER_EVENT_ERR:
        log_info("mix_music: AUDIO_SERVER_EVENT_ERR");
    case AUDIO_SERVER_EVENT_END:
        log_info("mix_music: AUDIO_SERVER_EVENT_END");
...
static int app_music_play_mix_file(const char *path)
{
    ...
    log_info("play_mix_music_file: suss");
...
__err:
    ...
    log_error("play_mix_music_file: fail : %d", err);
...
static int local_music_dec_file(void *file, int breakpoint, void *handler, int arg)
{
    ...
    log_info("app_music_dec_local_file");
...
static int local_music_dec_switch_file(int fsel_mode)
{
    ...
    log_info("app_music_dec_switch_file");
...
static int local_music_dec_switch_dir(int fsel_mode)
{
    ...
    log_info("app_music_dec_switch_dir");
...
__again:
    do {
        ...
        if (fsel_mode == FSEL_NEXT_FILE) {
            ...
            log_info("fselect_dir_faild, create dir");
            return -ENOENT;
        }
    ...
    if (!file) {
        if (i >= __this->dir_list->file_number) {
            fclose(dir);
            log_info("fselect_no_purpose_dir, create dir");
            local_music_create_new_dir();
            return -ENOENT;
        }
        if (fsel_mode == FSEL_FIRST_FILE) {
...
static int local_music_dec_seek(int seek_step)
{
    ...
    if (__this->total_time != 0 && __this->total_time != -1) {
        if (__this->play_time + seek_step <= 0 || __this->play_time + seek_step >= __this->total_time) {
            log_error("local music seek out of range");
            return -1;
        }
    }
    ...
    log_info("local music seek step : %d", seek_step);
...
static int app_music_switch_local_device(const char *path)
{
    log_info("app_music_switch_local_device");
...
static int __net_download_ready(void)
{
...
static int net_music_dec_stop(int save_breakpoint)
{
    ...
    log_info("net_music_dec_stop: %d", save_breakpoint);
        ...
        if (save_breakpoint) {
            ...
            if (__this->net_bp.url) {
                ...
                if (!err) {
                    ...
                    log_info("save_breakpoint: fptr=%x", __this->net_bp.fptr);
...
static int net_music_dec_switch_file(int fsel_mode)
{
    ...
    log_info("net_music_dec_switch_file");
...
{
    log_info("net_music_dec_breakpoint");
...
static int net_music_dec_seek(int seek_step)
{
    ...
    log_info("net music seek step : %d", seek_step);
...
static int alarm_loop_handler(int arg)
{
    ...
    if (local_alarm_flag) {
        ...
        if (!file) {
            log_error("fopen alarm file failed");
            return -1;
        }
...
static void ai_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case AI_SERVER_EVENT_CONNECTED:
        if (!strcmp((const char *)argv[1], "dlna")) {
            break;
        }
...
static int app_music_ai_listen_start(u8 voice_mode, u8 enable_vad)
{
    ...
    if (err) {
        ...
        log_error("-------%s-------%d--err = %d", __func__, __LINE__, err);
        return err;
    }
...
static void app_music_event_net_connected(void)
{
    union ai_req req = {0};
...
static void dec_server_event_handler(void *priv, int argc, int *argv)
{
    switch (argv[0]) {
    case AUDIO_SERVER_EVENT_ERR:
        log_info("app_music: AUDIO_SERVER_EVENT_ERR");
    case AUDIO_SERVER_EVENT_END:
        log_info("app_music: AUDIO_SERVER_EVENT_END");
        if (__this->cmp_file != (void *)argv[1]) {
            log_warn("not the same file, may be closed!");
            break;
        }
        /* os_time_dly(3); */
        union audio_req r = {0};
...
static int __play_voice_prompt(const char *fname, void *dec_end_handler, int save_bp)
{
    ...
    log_info("play_voice_prompt: %s", fname);
...
static int app_music_main(void)
{
    int err = 0;

    log_info("action_music_play_main");
    ...
    if (!__this->dec_server) {
        log_error("play_music open audio_server fail!");
        return -EPERM;
    }
...
static int play_mode_switch(void)
{
    ...
    log_info("__this->local_mode = %d", __this->local_play_mode);
    return 0;
}
...
{
#if TCFG_POWER_ON_ENABLE_BT && defined CONFIG_NET_ENABLE
