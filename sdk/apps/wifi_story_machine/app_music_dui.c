...
/* #define CONFIG_PWM_LED_ENABLE */
#define CONFIG_PLAY_PROMPT_DISABLE_KEY  //播放提示音时屏蔽按键
...
#ifdef CONFIG_PLAY_PROMPT_DISABLE_KEY
        key_event_disable();
#endif
...
static void ai_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case AI_SERVER_EVENT_RESUME_PLAY:
        log_info("__this->last_mode = %d", __this->last_mode);
...
#endif
                sys_power_auto_shutdown_resume();
#if TCFG_USER_BLE_ENABLE && (TCFG_BT_NET_CFG_EN || TCFG_BT_NET_CFG_DUI_EN) && TCFG_POWER_ON_ENABLE_BLE
            } else {
                app_music_play_voice_prompt("SsidRecv.mp3", NULL);
                config_network_connect();
