...
//设置音量大小
int local_music_set_dec_volume(int value)
{
...
int local_music_dec_switch_file(int fsel_mode)
{
    ...
    if (!__this->fscan) {
        extern void post_msg2player_name(const char *name);
        post_msg2player_name("No Music Files Found");
        log_info("fscan err");
        return -1;
    }
    if (!__this->fscan->file_number) {
        extern void post_msg2player_name(const char *name);
        post_msg2player_name("No Music Files Found");
        log_info("no files");
        return -1;
...
__retry:
    do {
        ...
        if (file) {
            ...
            if (len > 0) {
                extern void post_msg2player_name(const char *name);
                post_msg2player_name(name);
                log_info("play file name : %s", name);
            } else {
                extern void post_msg2player_name(const char *name);
                //post_msg2player_name(" ");
            }
            ++i;
            break;
        }
...
static void dec_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case AUDIO_SERVER_EVENT_CURR_TIME:
        ...
        int val = ((float)__this->play_time / __this->total_time) * 100;
        extern void post_msg2player_time(int time);
        post_msg2player_time(val);
...
int local_music_mode_init(void)
{
    ...
    server_register_event_handler_to_task(__this->dec_server, NULL, dec_server_event_handler, "lvgl_v8_main_task");
...
#else
        return local_music_switch_local_device(CONFIG_VOICE_PROMPT_FILE_PATH);
#endif
    }
}
