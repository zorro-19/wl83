...
{
    gui_msg_sub_t *sub;
    sub = gui_msg_create_sub(GUI_RECORDER_VOLUME_MSG_ID_VOLUME);
    if (sub != NULL) {
...
void gui_recorder_volume_msg_init_events()
{
    ...
    _gui_msg_status_t status[1] = {
        {GUI_RECORDER_VOLUME_MSG_ID_VOLUME, 0, 0},
    };
    ...
    lv_subject_t *subject_volume = gui_msg_get_subject(GUI_RECORDER_VOLUME_MSG_ID_VOLUME);
    if (!guider_ui.audio_recorder_del) {
        gui_recorder_volume_msg_volume_cb(GUI_MSG_ACCESS_GET, &guider_msg_data, VALUE_INT);
        lv_subject_add_observer_obj(subject_volume, gui_msg_set_slider_starting_value_by_int32_cb, guider_ui.audio_recorder_enc_gain_slider, &guider_msg_data);


        for (int i = 0; i < 1; i++) {
            if (status[i].msg_id == GUI_RECORDER_VOLUME_MSG_ID_VOLUME) {
...
gui_msg_data_t *gui_recorder_volume_msg_get(int32_t msg_id)
{
    switch (msg_id) {
