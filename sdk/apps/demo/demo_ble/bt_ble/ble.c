...
static const u8 test_remoter_name1[] = "JL-AC79XX-0078(BLE)";//
...
static void client_event_callback(le_client_event_e event, u8 *packet, int size)
{
    ...
    case CLI_EVENT_MATCH_DEV:
        log_info("match_name:%s", ((client_match_cfg_t *)packet)->compare_data);
        break;
    ...
    case CLI_EVENT_SEARCH_PROFILE_COMPLETE:
        ...
        //可以开始发送数据
#if TCFG_TRANS_MULTI_BLE_MASTER_NUMS
        for (int i = 0; i < TCFG_TRANS_MULTI_BLE_MASTER_NUMS; i++) {
