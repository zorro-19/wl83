...
void bt_set_emitter_enable_flag(bool flag);
...
{
    bd_addr_t remote_addr[CFG_REMOTE_DN_END - CFG_REMOTE_DN_00 + 1];
    memset(remote_addr, 0, sizeof(remote_addr));
    u8 flag = bt_restore_remote_device_info_opt(remote_addr, CFG_REMOTE_DN_END - CFG_REMOTE_DN_00 + 1, get_remote_dev_info_index());
    for (u8 i = 0; i < flag; i++) {
...
void bt_emitter_esco_pp(u8 pp)
{
    ...
    if (pp) {
        ...
        if (bt_get_call_status() == BT_CALL_INCOMING) {
            ...
            if (last_server) {
                bt_emitter_a2dp_resume_timer = sys_timeout_add((void *)1, bt_emitter_a2dp_resume, 500);
            }
        }
    }
}
