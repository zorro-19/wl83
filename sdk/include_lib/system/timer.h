 ...
 * @note    在定时时间到之前进行重置之后重新计时,sys_timeout定时到之后会被删除,re_run无效
 */
 ...
 * @param	priv	私有参数
 * @param	func	回调函数
 * @param	usec	定时时间， 单位：微秒
 * @param	call_in_irq	回调函数是否在中断里面执行
 * @param	once	是否只运行一次
 * @return  定时器分配的id号
 * @note 	微秒级定时器同一时间只能运行一个
 */
int sys_usec_timer_add_to_task(const char *task, void *priv, void (*func)(void *priv), u32 usec, u8 call_in_irq, u8 once);
...
/**
 * @brief   微秒级定时器的定时时间修改
 * @param	id	sys_usec_timer_add分配的id号
 * @param	usec	定时时间， 单位：微秒
 */
