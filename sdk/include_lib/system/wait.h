...
/**
 * @brief 条件轮询回调,回调函数将通过队列消息由指定线程接收执行,超时会回调
 * @param task_name:        指定执行回调函数的任务名
 * @param condition:        条件判断函数指针
 * @param callback:         回调函数指针
 * @param priv:             传到回调函数参数指针
 ...
 * @return id:创建成功的id号 0:失败
 */
u16 wait_completion_timeout_add_to_task(const char *task_name, int (*condition)(void *), int (*callback)(void *, int timeout), void *priv, void *condition_priv, u32 msec);
