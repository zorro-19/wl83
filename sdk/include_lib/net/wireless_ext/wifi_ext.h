...
void wpa_supplicant_set_cb(int (*cb)(void *, enum wpa_supplicant_state, void *), void *priv);
int wifi_module_init(struct WIFI_INIT_INFO *info);
void wifi_module_remove(void);

int ext_wifi_module_enter_ap_mode(char *ssid, char *pwd);
void ext_wifi_module_exit_ap_mode(void);
int ext_wifi_module_enter_sta_mode(char *ssid, char *pwd);
...
void ext_wpa_supplicant_uninit(void);
void ext_wpa_supplicant_req_scan_ssid(void *wpa_s, int sec);

int wifi_module_is_init(void);
u32 ext_wifi_get_upload_rate(void);

u32 ext_wifi_get_download_rate(void);
...
int ext_wifi_pm_suspend(void);
int ext_wifi_pm_resume(void);
#endif  /*WIFI_EXT_H*/
