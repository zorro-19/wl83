...
static void wired_ethernetif_input(struct netif *netif);
//6字节的目的MAC＋6字节的源MAC＋2字节的帧类型＋最大数据包(1500)
//unsigned char Rx_Data_Buf[1514];
//unsigned char Tx_Data_Buf[1514];
u8 *oeth_get_txaddr(void);
int oeth_get_rxpkt_addr_len(const struct eth_platform_data *data, struct oeth_data *_oeth_data);
...
wired_low_level_init(struct netif *netif)
{
    ...
    const struct eth_platform_data *data = get_eth_platform_data();
    if (data == NULL) {
    ...
    memcpy(netif->hwaddr, data->mac_addr, 6);
    ...
    create_eth_rx_task(netif, data->task_prio);
}
